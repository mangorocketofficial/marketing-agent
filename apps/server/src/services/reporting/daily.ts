...
export interface MarketingDailyReportPayload {
  id: string;
  customerId: string;
  type: 'marketing-daily';
  periodStart: string;
  periodEnd: string;
...
export interface DailyReportResult {
  report: MarketingDailyReportPayload;
...
function resolvePeriod(input?: Pick<CreateDailyReportInput, 'periodStart' | 'periodEnd'>): {
  periodStart: string;
  periodEnd: string;
} {
...
function buildSummaryText(channelStats: ChannelStats[], insights: string[], recommendations: string[]): string {
  ...
  const parts = [
    `일간 발행 ${totalPosts}건 (성공 ${totalSuccess} / 실패 ${totalFail}), 성공률 ${successRate}%`,
    insights[0],
    recommendations[0],
...
async function insertReport(db: DatabaseClient, report: MarketingDailyReportPayload): Promise<void> {
...
export async function createDailyMarketingReport(
  db: DatabaseClient,
  ...
  const [channelStats, publishingStats, competitorResult] =
    await Promise.all([
  ...
  const report: MarketingDailyReportPayload = {
    id: randomUUID(),
    customerId,
    type: 'marketing-daily',
    periodStart,
